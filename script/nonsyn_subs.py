...
def get_ancestral_alleles(seqs, threashold):
	...
	for s in range(len(seqs[0])):
		...
		AA_freqs = [0 for i in range(len(AA))]
		
		for seq in seqs:
			AA_freqs[AA_freqs.index(seq[s])] += 1
		...
		for i in range(len(AA_freqs)):
			if AA_freqs[i] > threashold:
				ancestral.append((AAs[i], AA_freqs[i]))
				found = 1
				break
...
def find_substitutions(seqs_by_season):
	...
	for season in range(len(seqs_by_season)):
		...
		if season == 0: 
			...
			threashold = 0.95
		ancestral_each_site = get_ancestral_alleles(seqs_by_season[season], threashold)
		ancestrals_by_season.append(ancestral_each_site)
		
		#Determine substitutions at each site
		for s in range(len(ancestral_each_site)):	
			...
			if ancestral_each_site[s][0] != "-" and ancestral_each_site[s][0] != ancestral_by_season[season-1][s][0]:
				subs[season].append((ancestral_by_season[season-1][s][0], s+1, ancestral_each_sites[s][0]))
...
def put_seqs_by_year(infName, beginY, endY):
	...
	for line in inf:
		if line.find(">") >= 0:
			...
			seqs_by_year[year].append(line.split("\n")[0])
			
	inf.close()
	return seqs_by_year
	
dataDir = os.path.normpath("C:/meaningful folders/Nonsyn_subs/data/")
#from 1968 to 1992, put sequences by year
seqs_by_year = (dataDir+"\\"+"ncbi_aligned_6812_AA.fas", 1968, 1992)
...
for season in range(len(seqs_by_season)):
	print ( season + 1968, len(seqs_by_season[season]) )
